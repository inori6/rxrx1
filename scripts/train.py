...
def run_training(config, epoch_callback=None):
    ...
    try:
        ...
        batch_config = (
            (transform_config.get("batch") or []) if transform_config.get("switch") else []
        )
        if batch_config and getattr(criterion, "metric_enabled", False):
            raise ValueError(
                "Metric supervision is incompatible with MixUp/CutMix; set transform.batch: []."
            )
        ...
        train_acc_definition = (
            "mixed_target_weight_at_prediction" if batch_config else "hard_label_accuracy"
        )
        ...
        train_normalizer = build_normalizer(
            config, split="train", project_root=project_root, logger=logger
        )

        normalization_config = config.get("normalization") or {}
        reference_config = normalization_config.get("reference") or {}
        ...
        share_train_stats_with_val = train_reference_stats is not None and split_policy in {
            "train_only",
            "all",
        }
        ...
        val_normalizer = build_normalizer(
            config,
            stats=(train_reference_stats if share_train_stats_with_val else None),
            split="val",
        ...
        checkpoint_path = (
            project_root / config["checkpoint"]["dir"] / config["experiment"]["name"] / "best.pt"
        )
