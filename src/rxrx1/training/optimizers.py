...
def _build_discriminative_groups(model, base_lr, lr_ratio, fusion_lr_ratio=3.0):
    if not hasattr(model, "features"):
        raise ValueError("Discriminative learning rates require a model with model.features.")

    if not hasattr(model, "classifier"):
        raise ValueError("Discriminative learning rates require model.classifier.")

    if len(model.features) < 9:
        raise ValueError("Expected EfficientNet features to contain at least 9 stages.")
    ...
    if getattr(model, "projection", None) is not None:
    ...
    fusion_params = list(fusion.parameters()) if fusion is not None else []
    if fusion_params:
        parameter_groups.append(
            {"params": fusion_params, "lr": base_lr * fusion_lr_ratio, "group_name": "fusion"}
        )
...
def build_optimizer(model, config):
    ...
    return torch.optim.AdamW(
        model.parameters(), lr=float(optimizer_config["lr"]), weight_decay=weight_decay
    )
